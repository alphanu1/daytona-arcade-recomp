...
| MAME `src/devices/cpu/i960/i960.cpp` (instruction semantics) | same | same | BSD-3-Clause | **Code transplanted** into `src/runtime/i960_core.cpp` (the runtime's i960 core), BSD-3 notice kept in the file. MAME's semantics are used verbatim so the runtime diffs cleanly against MAME in lockstep. Licence-compatible with any project licence. | Device framework, cycle accounting and burst-stall machinery removed; memory through `rt::Bus`; `fatalerror` throws; interrupt-taken hook added. Every change is marked in the file. |
