...
// Encodings MAME's disassembler rejects but its executor still runs. The
// hardware behaviour for these is unconfirmed; every one seen in Daytona's
// code is a finding.
enum Quirk : uint8_t {
    kQuirkNone = 0,
...
struct Insn {
    ...
    bool executable() const { return op && op->mame_exec && ea != EaMode::Reserved; }
