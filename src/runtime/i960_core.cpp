...
//
// i960 instruction semantics transplanted from MAME's
// src/devices/cpu/i960/i960.cpp at dddd73680656e355bb2b5beecab1167c9f07bf81
// (BSD-3-Clause; notice above kept as the licence requires). Changes: the
// device framework, cycle accounting (m_icount is kept but unused) and the
// burst-stall machinery are removed; memory goes through I960Core::bus;
// fatalerror/logerror map to the runtime. The instruction semantics are
// MAME's, unchanged, so the runtime can be diffed against MAME in lockstep.
// See THIRD_PARTY.md.
...
uint32_t I960Core::i960_read_dword_unaligned(uint32_t address)
{
...
std::pair<uint32_t, uint16_t> I960Core::i960_read_dword_unaligned_flags(uint32_t address)
{
	if (!DWORD_ALIGNED(address)) {
...
uint16_t I960Core::i960_read_word_unaligned(uint32_t address)
{
...
void I960Core::i960_write_dword_unaligned(uint32_t address, uint32_t data)
{
	if (!DWORD_ALIGNED(address))
	{
...
uint16_t I960Core::i960_write_dword_unaligned_flags(uint32_t address, uint32_t data)
{
	if (!DWORD_ALIGNED(address))
	{
...
void I960Core::i960_write_word_unaligned(uint32_t address, uint16_t data)
{
	if (!WORD_ALIGNED(address))
	{
		bus->write_byte(address, data & 0xff);
...
void I960Core::send_iac(uint32_t adr)
{
	uint32_t iac[4];
...
uint32_t I960Core::get_ea(uint32_t opcode)
{
...
uint32_t I960Core::get_1_ri(uint32_t opcode)
{
...
uint32_t I960Core::get_2_ri(uint32_t opcode)
{
...
uint64_t I960Core::get_2_ri64(uint32_t opcode)
{
	if(!(opcode & 0x00001000))
...
void I960Core::set_ri(uint32_t opcode, uint32_t val)
{
...
void I960Core::set_ri2(uint32_t opcode, uint32_t val, uint32_t val2)
{
	if(!(opcode & 0x00002000))
	{
		m_r[(opcode>>19) & 0x1f] = val;
...
void I960Core::set_ri64(uint32_t opcode, uint64_t val)
{
	if(!(opcode & 0x00002000)) {
		m_r[(opcode>>19) & 0x1f] = val;
...
double I960Core::get_1_rif(uint32_t opcode)
{
	if(!(opcode & 0x00000800))
...
double I960Core::get_2_rif(uint32_t opcode)
{
	if(!(opcode & 0x00001000))
...
void I960Core::set_rif(uint32_t opcode, double val)
{
	if(!(opcode & 0x00002000))
...
double I960Core::get_1_rifl(uint32_t opcode)
{
	if(!(opcode & 0x00000800)) {
...
double I960Core::get_2_rifl(uint32_t opcode)
{
	if(!(opcode & 0x00001000)) {
...
void I960Core::set_rifl(uint32_t opcode, double val)
{
	if(!(opcode & 0x00002000)) {
		uint64_t v = d2u(val);
...
uint32_t I960Core::get_1_ci(uint32_t opcode)
{
	if(!(opcode & 0x00002000))
...
uint32_t I960Core::get_2_ci(uint32_t opcode)
{
	return m_r[(opcode >> 14) & 0x1f];
}

uint32_t I960Core::get_disp(uint32_t opcode)
{
	return sext(opcode, 24) - 4;
}

uint32_t I960Core::get_disp_s(uint32_t opcode)
{
	return sext(opcode, 13) - 4;
}

void I960Core::cmp_s(int32_t v1, int32_t v2)
{
	m_AC &= ~7;
	if(v1<v2)
		m_AC |= 4;
...
void I960Core::cmp_u(uint32_t v1, uint32_t v2)
{
	m_AC &= ~7;
	if(v1<v2)
		m_AC |= 4;
...
void I960Core::concmp_s(int32_t v1, int32_t v2)
{
	m_AC &= ~7;
	if(v1 <= v2)
		m_AC |= 2;
...
void I960Core::concmp_u(uint32_t v1, uint32_t v2)
{
	m_AC &= ~7;
	if(v1 <= v2)
		m_AC |= 2;
...
void I960Core::cmp_d(double v1, double v2)
{
	m_AC &= ~7;
	if(v1<v2)
		m_AC |= 4;
...
void I960Core::bxx(uint32_t opcode, int mask)
{
	if(m_AC & mask) {
...
void I960Core::fxx(uint32_t opcode, int mask)
{
	if(m_AC & mask) {
...
void I960Core::bxx_s(uint32_t opcode, int mask)
{
	if(m_AC & mask) {
...
void I960Core::test(uint32_t opcode, int mask)
{
	if(m_AC & mask)
...
double I960Core::round_to_int(double val)
{
	// apply rounding mode
	switch ((m_AC >> 30) & 3)
	{
...
void I960Core::take_interrupt(int vector, int lvl)
{
...
void I960Core::check_immediate_irqs()
{
...
void I960Core::check_pending_irqs()
{
	int int_tab = bus->read_dword(m_PRCB + 20);    // interrupt table
	int cpu_pri = (m_PC >> 16) & 0x1f;
...
void I960Core::do_call(uint32_t adr, int type, uint32_t stack)
{
	int i;
	uint32_t FP;
...
void I960Core::burst_stall_save(uint32_t t1, uint32_t t2, int index, int size, bool iswriteop)
{
	m_stall_state.t1 = t1;
	m_stall_state.t2 = t2;
...
void I960Core::execute_op(uint32_t opcode)
{
	uint32_t t1, t2;
	double t1f, t2f;
...
void I960Core::execute_set_input(int irqline, int state)
{
...
// ---------------------------------------------------------------------------
// Runtime additions (not from MAME).

// One pass of MAME's execute_run loop body (burst-stall path removed: the
// runtime never stalls).
void I960Core::execute_one()
{
	m_PIP = m_IP;
